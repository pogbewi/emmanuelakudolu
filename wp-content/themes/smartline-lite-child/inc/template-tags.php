...
function smartline_meta_date() {
	...
	$time_string = sprintf( '<a href="%1$s" title="%2$s" rel="bookmark"> %2$s </a>',
		esc_url( get_permalink() ),
		esc_html(get_the_time('F j, Y'))
	);

	echo '<span class="meta-date"> Posted On:' . $time_string . '</span>';
}
...
function smartline_display_footer_text() {
	...
	<span class="credit-link">
		 Copyright <?php echo savy_grid_video_site__copyright(); ?> Emmanuel Akudolu <<>> All Rights Reserved
	</span>
...
if ( ! function_exists( 'savy_grid_video_site__copyright' ) ) {
    function savy_grid_video_site__copyright(){
    global $wpdb;
